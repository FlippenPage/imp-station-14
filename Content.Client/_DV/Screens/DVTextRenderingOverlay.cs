    ...
    protected override void Draw(in OverlayDrawArgs args)
    {
        ...
        while (_queue.TryDequeue(out var queued))
        {
            ...
            foreach (var row in queued.Entity.Comp.Rows)
            {
                ...
                var texWrapSampleParameters = new TextureSampleParameters { WrapMode = TextureWrapMode.Repeat, Filter = true };

                if (row.Texture is null || row.Texture.Size != dimensionsInt)
                {
                    row.Texture = _clyde.CreateRenderTarget(dimensionsInt,
                        new RenderTargetFormatParameters(RenderTargetColorFormat.Rgba8), texWrapSampleParameters,
