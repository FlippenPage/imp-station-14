...
public sealed partial class ContestsSystem : EntitySystem
{
    ...
    {
        if (!_cfg.GetCVar(EinsteinCCVars.DoContestsSystem)
            || !_cfg.GetCVar(EinsteinCCVars.DoMassContests)
        ...
        return ContestClamp(ContestClampOverride(bypassClamp)
            ...
            : Math.Clamp(performerPhysics.Mass / otherMass,
                1 - _cfg.GetCVar(EinsteinCCVars.MassContestsMaxPercentage) * rangeFactor,
                1 + _cfg.GetCVar(EinsteinCCVars.MassContestsMaxPercentage) * rangeFactor));
    }
    ...
    {
        if (!_cfg.GetCVar(EinsteinCCVars.DoContestsSystem)
            || !_cfg.GetCVar(EinsteinCCVars.DoStaminaContests))
            return 1f;
    ...
    {
        if (!_cfg.GetCVar(EinsteinCCVars.DoContestsSystem)
            || !_cfg.GetCVar(EinsteinCCVars.DoHealthContests)
    ...
    // {
    //     if (!_cfg.GetCVar(EinsteinCCVars.DoContestsSystem)
    //         || !_cfg.GetCVar(EinsteinCCVars.DoMindContests))
    //         return 1f;
    ...
    // {
    //     if (!_cfg.GetCVar(EinsteinCCVars.DoContestsSystem)
    //         || !_cfg.GetCVar(EinsteinCCVars.DoMoodContests)
